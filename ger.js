...
const tombolSewaScriptAktif = await new Promise((resolve) => {
  ...
  const messageText = `
━━━━━━━━━━━━━━━━━━━━━━
Selamat datang di <b>BOT ${NAMA_STORE}</b> 🚀
━━━━━━━━━━━━━━━━━━━━━━
Bot otomatis untuk membeli akun VPN dengan mudah & cepat.
...
────────────────────────────────────
🕒 <b>Waktu</b>   : ${timeNow} WIB
📅 <b>Tanggal</b> : ${currentDay}, ${currentDate}
🖥️ <b>Server</b>  : ${jumlahServer} | 👥 User: ${jumlahPengguna}
♻️ <b>Runtime</b> : ${uptimeFormatted}

☎️ <b>Admin</b>   : <a href="https://t.me/frel01">@frel01</a>`;
...
keyboard.push([
  ...
  { text: '🗑️ Hapus Akun', callback_data: 'service_delete' }
]);
...
bot.action(/^(delete)_username_(vmess|vless|trojan|shadowsocks|ssh)_(.+)$/, async (ctx) => {
...
async function handleServiceAction(ctx, action) {
  ...
  if (action === 'trial') {
    ...
    keyboard = [
      [{ text: '🗑️ SSH', callback_data: 'delete_ssh' }],
      [{ text: '🗑️ Vmess', callback_data: 'delete_vmess' }, { text: '🗑️ Vless', callback_data: 'delete_vless' }],
      [{ text: '🗑️ Trojan', callback_data: 'delete_trojan' }, { text: '🗑️ Shadowsocks', callback_data: 'delete_shadowsocks' }],
      [{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]
    ];
  }
...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    // Di dalam fungsi startSelectServer, tambahkan case untuk action 'delete'
if (action === 'trial') {
...
bot.on('text', async (ctx, next) => {
    ...
    // Di dalam bot.on('text'), tambahkan case untuk menghapus akun
if (state && state.step.startsWith('username_delete_')) {
  ...
  try {
    ...
    logger.error(`❌ Error menghapus akun ${type}:`, error);
    msg = '❌ *Gagal menghapus akun. Silahkan coba lagi nanti.*';
  } finally {
    clearInterval(intervalId);
    try {
      await ctx.telegram.deleteMessage(ctx.chat.id, waitMsgId);
    } catch (e) {
      logger.warn(`Gagal menghapus pesan loading untuk user ${ctx.from.id}: ${e.message}`);
    }
  }
